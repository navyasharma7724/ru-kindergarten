...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        for (int i = 0; i < numberOfLines; i++) {
            String studentFirstName = StdIn.readString();
            String studentLastName = StdIn.readString();
            int studentAge = StdIn.readInt();
            Student student = new Student(studentFirstName , studentLastName , studentAge);
            SNode studentNode = new SNode(student, null);
            if (studentsInLine == null) {
               studentsInLine = studentNode; 
            }
            else {
                studentNode.setNext(studentsInLine);
                studentsInLine = studentNode; 
            }
        }
    ...
    public void setupSeats(String seatingChart) {
        ...
        int numberOfRows = StdIn.readInt();
        int numberOfColumns = StdIn.readInt();
        seatingLocation = new boolean[numberOfRows][numberOfColumns]; //initialize seatingLocation array
        studentsSitting = new Student[numberOfRows][numberOfColumns]; //initialize studentsSitting array
        for (int i = 0; i < numberOfRows; i++) {
            for (int j = 0; j < numberOfColumns; j++) {
                boolean studentIsSitting = StdIn.readBoolean(); 
                if (studentIsSitting == false) {
                    seatingLocation[i][j] = false;
                } else if (studentIsSitting == true) {
    ...
    public void seatStudents () {
        // WRITE YOUR CODE HERE
        // If a space in the studentsSitting array is both true and null, then that 
        // means that that space is unoccupied and that we need to fill it with a 
        // student from the LL.
        // We then need to update the front of the linked list because 
        // each node represents a student and the first node in the LL, the 
        // 'front,' is the student that will be placed in the studentsSitting array. 
    ...
    public void insertMusicalChairs () {
        // WRITE YOUR CODE HERE
        SNode pointerFirst = new SNode(null, null);
        SNode pointerLast = new SNode(null, null);
        for (int i = 0; i < studentsSitting.length; i++) {
            for (int j = 0; j < studentsSitting[0].length; j++) {
                if (studentsSitting[i][j] != null) {
                    SNode studentInMusicalChair = new SNode(studentsSitting[i][j], null);
                    studentsSitting[i][j] = null;
                    if (musicalChairs == null) {
                        musicalChairs = studentInMusicalChair;
                        pointerFirst = musicalChairs;
                        pointerLast = musicalChairs;
                        musicalChairs.setNext(musicalChairs);
                    } else {
                        pointerFirst = musicalChairs.getNext();
                        pointerLast.setNext(studentInMusicalChair);
                        pointerLast = studentInMusicalChair;
                        studentInMusicalChair.setNext(pointerFirst);
                        musicalChairs = studentInMusicalChair;
                    }
                }
            }
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        SNode secondPointer = new SNode(null, null);
        ...
        Student studentToInsert = front.getStudent();
        if (front == musicalChairs.getNext()) {
            musicalChairs.setNext(front.getNext());
            front.setNext(null);
        } else if (front == musicalChairs) {
            secondPointer.setNext(front.getNext());
            front.setNext(null);
            musicalChairs = secondPointer;
        } else {
            secondPointer.setNext(front.getNext());
            front.setNext(null);
            }
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        if ( pointer == null ) {
            ...
            pointer = studentsInLine;
        } else {
            previous = null; 
            while ((pointer != null) && (studentToInsert.getHeight() >= pointer.getStudent().getHeight())) {
                previous = pointer;
                pointer  = pointer.getNext();
            }
            if (previous == null) {
                studentsInLine = new SNode(studentToInsert, pointer); 
            } else {
                SNode newSNode = new SNode(studentToInsert, pointer);
    ...
    public void eliminateLosingStudents() {
        ...
        int musicalChairsSize = n;
        while (musicalChairsSize != 1) {
            moveStudentFromChairsToLine(musicalChairsSize);
            musicalChairsSize--;
        }
    }
